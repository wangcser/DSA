...
![img](https://wizardforcel.gitbooks.io/the-art-of-programming-by-july/images/9/9.3/9.3.1.jpg)
...
![img](https://wizardforcel.gitbooks.io/the-art-of-programming-by-july/images/9/9.3/9.3.2.jpg)
...
![img](https://wizardforcel.gitbooks.io/the-art-of-programming-by-july/images/9/9.3/9.3.3.jpg)

#### 1.2、错误率估计

前面我们已经提到了，Bloom Filter在判断一个元素是否属于它表示的集合时会有一定的错误率（false positive rate），下面我们就来估计错误率的大小。在估计之前为了简化模型，我们假设kn1</sub>, x2,…,xn}的所有元素都被k个哈希函数映射到m位的位数组中时，这个位数组中某一位还是0的概率是：

![img](http://chart.apis.google.com/chart?cht=tx&chl=p%27=\\left(1-\\frac{1}{m}\\right)^{kn}\approx e^{-kn/m})
...
![img](Bloom Filter.assets/chart.png)

令ρ为位数组中0的比例，则ρ的数学期望E(ρ)= p’。在ρ已知的情况下，要求的错误率（false positive rate）为：

![img](Bloom Filter.assets/chart-1563587217137.png)
...
![img](Bloom Filter.assets/chart-1563587217218.png)^k=(1-p')^k)

![img](Bloom Filter.assets/chart-1563587217161.png)
...
![img](Bloom Filter.assets/chart-1563587217167.png)
...
![img](Bloom Filter.assets/chart-1563587217220.png)
...
![img](Bloom Filter.assets/chart-1563587217219.png)

个集合。m位的位数组共有2m个不同的组合，进而可以推出，m位的位数组可以表示

![img](Bloom Filter.assets/chart-1563587217236.png)

个集合。全集中n个元素的集合总共有

![img](Bloom Filter.assets/chart-1563587217241.png)
...
![img](http://chart.apis.google.com/chart?cht=tx&chl=m\\geq\\log_2\\frac{C_{u}^n}{C_{n%2B\\epsilon(u-n)}^n}\approx\log*2\frac{C*{u}^n}{C_{\epsilon u}^n}\geq\log_2\epsilon^{-n}=n\log_2(1/\epsilon))
...
![img](http://chart.apis.google.com/chart?cht=tx&chl=m\\geq%20n\\frac{\\log_2(1/\\epsilon)}{\ln 2}=n\log_2\log_2(1/\epsilon))
